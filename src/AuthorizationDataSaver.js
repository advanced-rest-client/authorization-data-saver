 ...
 */
export class AuthorizationDataSaver extends HeadersParserMixin(LitElement) {
  static get styles() {
    return css`
    :host {
      margin: 0;
  ...
  static get properties() {
    return {
       ...
       */
      _openedDialog: { type: String }
    };
  }

  get _openedDialog() {
    return this.__openedDialog;
  }

  set _openedDialog(value) {
    const old = this.__openedDialog;
    /* istanbul ignore if */
    if (old === value) {
      return;
    }
    this.__openedDialog = value;
    this._restoreDialogData();
  }
   ...
   */
  async _restoreDialogData() {
    const { url, _openedDialog } = this;
    if (!url || !_openedDialog) {
      return;
    }
    ...
    const authData = await e.detail.result;
    if (authData) {
      this._setRestoredAuthData(_openedDialog, authData);
    };
  }
  /**
